...
# Specific filtering arguments for certain models
...
def parse_args():
    ...
    # Arguments for 3D pose estimation
    parser.add_argument(
        '--pose3d',
        type=str,
        default=None,
...
# Function to display available model aliases
...
def main():
    ...
    # Display model aliases if requested
    if display_alias:
        ...
        # Determine total frames for progress bar if input is a video
        if call_args.get('inputs', '').endswith(('.mp4', '.avi')):
        ...
        with tqdm(total=total_frames, desc="Processing") as pbar:
            for result in inferencer(**call_args):
                if 'predictions' in result:  # MMPose >= 1.0 uses 'predictions' key
                    frame_keypoints = []
                    for person in result['predictions'][0]:
                        # Extract keypoints and scores
                        frame_keypoints.append({
                            'keypoints': person['keypoints'],  # Direct list access
        ...
        output_video_path = call_args.get('vis_out_dir', '')
        output_video_name = os.path.splitext(os.path.basename(call_args.get('inputs', '')))[0] + '.json'
